...
function Geyser.Label:setFont(font)
	...
	if not (af[font] or font == "") then
		local err = "Geyser.Label:setFont(): attempt to call setFont with font '"
			.. font
	...
	for i, v in pairs(parent.nestedLabels) do
		...
		if v.layoutDir == "V" and not scrollV then
			if layout[v.layoutDir] + v.get_height() <= maxDim[v.layoutDir] then
				if
					not scrollStartV
					or (entryCount[v.layoutDir] > scrollStartV and entryCount[v.layoutDir] < scrollEndV)
				then
		...
		elseif v.layoutDir == "H" and not scrollH then
			if layout[v.layoutDir] + v.get_height() <= maxDim[v.layoutDir] then
				if
					not scrollStartH
					or (entryCount[v.layoutDir] > scrollStartH and entryCount[v.layoutDir] < scrollEndH)
				then
					table.insert(nestedLabels[v.layoutDir], v)
...
local function addElement(self, name, configLabel, myMenu, depth, index, restyle)
	...
	local MenuStyle = myMenu.MenuLabels[name].stylesheet
		or configLabel["MenuStyle" .. depth]
		or configLabel["MenuStyle"]
