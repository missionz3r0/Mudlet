...
function Geyser.Gauge:setValue(currentValue, maxValue, text)
	...
	if self.orientation == "horizontal" then
		...
		self.front:resize(function()
			return math.floor((self.back.get_width() - totalBackOffset) * (gaugeValue / 100) + 0.5)
		end, function()
			return math.floor(self.back.get_height() - topOffset - bottomOffset + 0.5)
		end)
	elseif self.orientation == "vertical" then
		...
		local gaugeValue = self.value
		self.front:move(leftOffset .. "px", function()
			return topOffset + math.floor((self.back.get_height() - totalBackOffset) * (1 - gaugeValue / 100) + 0.5)
		end)
		self.front:resize(function()
			return math.floor(self.back.get_width() - leftOffset - rightOffset + 0.5)
		end, function()
			return math.floor((self.back.get_height() - totalBackOffset) * (gaugeValue / 100) + 0.5)
		end)
	elseif self.orientation == "goofy" then
		...
		local totalBackOffset = leftOffset + rightOffset
		local gaugeValue = self.value
		self.front:move(function()
			return leftOffset + math.floor((self.back.get_width() - totalBackOffset) * (1 - gaugeValue / 100) + 0.5)
		end, topOffset .. "px")
		self.front:resize(function()
			return math.floor((self.back.get_width() - totalBackOffset) * (gaugeValue / 100) + 0.5)
		end, function()
