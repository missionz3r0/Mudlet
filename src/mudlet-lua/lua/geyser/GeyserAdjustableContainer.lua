...
function Adjustable.Container:setTitle(text, color, format)
	...
	if
		...
		and (
			...
			or self.lockStyle == "standard"
			or self.lockStyle == "border"
			or self.lockStyle == "full"
		)
	then
		return
	end
...
function Adjustable.Container:onMove(label, event)
	...
	if adjustInfo.x and adjustInfo.name == label.name then
		...
		local hasScrollBox = self.windowname
			and Geyser.parentWindows
			and Geyser.parentWindows[self.windowname]
			and Geyser.parentWindows[self.windowname].type == "scrollBox"
		...
		elseif adjustInfo.move == false then
			...
			self:move(tx, ty)
			local minw, minh = 0, 0
			if self.container == Geyser and not self.noLimit then
				minw, minh = 75, 25
			end
...
function Adjustable.Container:validAttachPositions()
	...
	if (winh * 0.8) - self.get_height() <= self.get_y() then
		found_positions[#found_positions + 1] = "bottom"
	end
	if (winw * 0.8) - self.get_width() <= self.get_x() then
		found_positions[#found_positions + 1] = "right"
	end
	if self.get_y() <= winh * 0.2 then
		found_positions[#found_positions + 1] = "top"
	end
	if self.get_x() <= winw * 0.2 then
		found_positions[#found_positions + 1] = "left"
	end
...
function Adjustable.Container:attachToBorder(border)
	...
	self:adjustBorder()
	self.resizeHandlerID = registerAnonymousEventHandler("sysWindowResizeEvent", function()
		self:resizeBorder()
	end)
	closeAllLevels(self.rCLabel)
end
...
function Adjustable.Container:detach()
	...
	self.attached = false
	if self.resizeHandlerID then
		killAnonymousEventHandler(self.resizeHandlerID)
	end
end
...
function Adjustable.Container:resetBorder(where)
	...
	if where == "right" then
		setBorderRight(resetTo)
	elseif where == "left" then
		setBorderLeft(resetTo)
	elseif where == "bottom" then
		setBorderBottom(resetTo)
	elseif where == "top" then
		setBorderTop(resetTo)
	end
end
...
function Adjustable.Container:save(slot, dir)
	assert(
		...
		"Adjustable.Container.save: bad argument #1 type (slot as string or number expected, got " .. type(slot) .. "!)"
	)
	assert(
		dir == nil or type(dir) == "string",
		"Adjustable.Container.save: bad argument #2 type (directory as string expected, got " .. type(dir) .. "!)"
	)
...
function Adjustable.Container:load(slot, dir)
	...
	assert(
		slot == nil or type(slot) == "string" or type(slot) == "number",
		"Adjustable.Container.load: bad argument #1 type (slot as string or number expected, got " .. type(slot) .. "!)"
	)
	assert(
		dir == nil or type(dir) == "string",
		"Adjustable.Container.load: bad argument #2 type (directory as string expected, got " .. type(dir) .. "!)"
	)
	dir = dir or self.defaultDir
	...
	if not ok then
		...
		debugc(
			string.format(
				"Adjustable.Container.load: Save file %s got corrupted. It was deleted so everything else can load properly.",
				loadDir
			)
		)
		return false
...
function Adjustable.Container:reposition()
	...
	raiseEvent(
		...
		self.get_x(),
		self.get_y(),
		adjustInfo.name == self.adjLabel.name
			and (adjustInfo.move or adjustInfo.right or adjustInfo.left or adjustInfo.top or adjustInfo.bottom)
	)
end
...
function Adjustable.Container:deleteSaveFile(dir)
	assert(
		dir == nil or type(dir) == "string",
		"Adjustable.Container.deleteSaveFile: bad argument #1 type (directory as string expected, got "
			.. type(dir)
			.. "!)"
	)
	dir = dir or self.defaultDir
...
function Adjustable.Container:setPercent(size_as_percent, position_as_percent)
	...
	if size_as_percent then
		self:resize(w, h)
	end
	if position_as_percent then
		self:move(x, y)
	end
end
...
function Adjustable.Container:enableAutoSave()
	self.autoSave = true
	self.autoSaveHandler = self.autoSaveHandler
		or registerAnonymousEventHandler("sysExitEvent", function()
			self:save()
		end)
end
...
function Adjustable.Container:new(cons, container)
	...
	me.minLabel.txt = me.minLabel.txt
		or [[<font size="5" face="Noto Emoji">🗕</font>]] .. self.Locale.min_restore.message
	...
	me.lockStylesLabel.txt = me.lockStylesLabel.txt
		or [[<font size="5" face="Noto Emoji">🖌</font>]] .. self.Locale.lockstyle.message
	me.customItemsLabel.txt = me.customItemsLabel.txt
		or [[<font size="5" face="Noto Emoji">🖇</font>]] .. self.Locale.custom.message
